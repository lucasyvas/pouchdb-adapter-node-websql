const {
  ...
  toPromise,
  uuid,
} = require("pouchdb-utils");
const { isDeleted, isLocalId } = require("pouchdb-adapter-utils");
const {
  collectConflicts,
  traverseRevTree,
  latest: getLatest,
} = require("pouchdb-merge");
const { safeJsonParse, safeJsonStringify } = require("pouchdb-json");
const {
  binaryStringToBlobOrBuffer: binStringToBlob,
  btoa,
} = require("pouchdb-binary-utils");

const parseHexString = require("./parseHex");
const websqlBulkDocs = require("./bulkDocs");

const { MISSING_DOC, REV_CONFLICT, createError } = require("pouchdb-errors");
...
const openDB = require("./openDatabase");
...
var DOC_STORE_AND_BY_SEQ_JOINER =
  ...
  function runMigration2(tx, callback) {
    ...
    tx.executeSql(
      ...
      function () {
        ...
        tx.executeSql(
          "ALTER TABLE " + DOC_STORE + " ADD COLUMN local TINYINT(1) DEFAULT 0",
          [],
          function () {
            tx.executeSql(
    ...
    tx.executeSql(local, [], function () {
      ...
      tx.executeSql(sql, [], function (tx, res) {
        ...
        function doNext() {
          ...
          tx.executeSql(
            "INSERT INTO " + LOCAL_STORE + " (id, rev, json) VALUES (?,?,?)",
            [row.id, rev, row.data],
            function (tx) {
              tx.executeSql(
                "DELETE FROM " + DOC_STORE + " WHERE id=?",
                [row.id],
                function (tx) {
                  tx.executeSql(
  ...
  function runMigration4(tx, callback) {
    function updateRows(rows) {
      function doNext() {
        ...
        var sql =
          "UPDATE " + BY_SEQ_STORE + " SET doc_id=?, rev=? WHERE doc_id_rev=?";
  ...
  function runMigration7(tx, callback) {
    ...
    tx.executeSql(sql, [], function (tx) {
      var sql =
        "UPDATE " +
        DOC_STORE +
        " SET max_seq=(SELECT MAX(seq) FROM " +
        BY_SEQ_STORE +
        " WHERE doc_id=id)";
      tx.executeSql(sql, [], function (tx) {
  ...
  function fetchVersion(tx) {
    ...
    tx.executeSql(sql, [], function (tx, result) {
      if (!result.rows.length) {
        ...
        tx.executeSql("SELECT db_version FROM " + META_STORE, [], function (
          tx,
          result
        ) {
